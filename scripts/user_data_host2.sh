...
cat << 'EOF' > /etc/nginx/conf.d/urbanreshape.ro.conf
server {
    listen 80;
    server_name urbanreshape.ro;
    ...
    access_log /var/log/nginx/urbanreshape.ro.access.log;
    error_log /var/log/nginx/urbanreshape.ro.error.log;
...
define( 'DB_COLLATE', '' );
define( 'WP_HOME', 'https://urbanreshape.ro' );
define( 'WP_SITEURL', 'https://urbanreshape.ro' );
